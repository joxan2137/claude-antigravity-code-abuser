...
<details>
<summary><b>Windows Auto-Setup 🚀</b></summary>

The easiest way to get started on Windows using the Antigravity provider is to use the included setup script:

```bat
# Just run this script to do everything automatically
setup.bat
```

This script will:
1. Generate an OAuth token for Antigravity.
2. Start the proxy server in the background.
3. Automatically configure Claude Desktop with the correct environment variables.
...
Add multiple Google accounts and the proxy will automatically rotate between them when one gets rate-limited (HTTP 429) or when the service is temporarily unavailable (HTTP 503).
...
Accounts are stored in `~/.freecc/antigravity_accounts.json`. The proxy loads them automatically on startup and rotates between accounts when rate limits or service errors are hit.

**Note on compatibility:** Antigravity includes a 6-pass message sanitizer to fix malformed conversation histories generated by clients (like Claude Desktop and Claude Code). It handles missing IDs, drops orphaned `tool_use`/`tool_result` blocks, collapses consecutive same-role turns, strings empty blocks, and enforces strict placement of tool calls to satisfy Google GenAI API requirements.
